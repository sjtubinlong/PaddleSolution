...
namespace PaddleSolution {
    ...
    bool DetectionPreProcessor::single_process(const std::string& fname, float* data, int* ori_w, int* ori_h) {
        ...
        for (int h = 0; h < rh; ++h) {
            ...
            for (int w = 0; w < rw; ++w) {
                for (int c = 0; c < channels; ++c) {
                    ...
                    float pixel = static_cast<float>(ptr[im_index++]) / 255.0;
    ...
    bool DetectionPreProcessor::batch_process(const std::vector<std::string>& imgs, float* data, int* ori_w, int* ori_h) {
